...
description: "OpenClaw 通道接入：Google Chat（Chat API）。通过 Google Chat API Webhook 接收私信和空间消息。"
---
...
私有仪表板仍只允许 tailnet 访问：
`https://<node-name>.<tailnet>.ts.net:8443/`

在 Google Chat 应用配置中使用公共 URL（不带 `:8443`）。

> 注意：此配置重启后仍会保留。如需移除，运行 `tailscale funnel reset` 和 `tailscale serve reset`。
...
- 路径：`/googlechat` 到 `http://localhost:18789/googlechat`
- 默认规则：HTTP 404（Not Found）

## 工作原理

1. Google Chat 向网关发送 Webhook POST。每个请求包含一个 `Authorization: Bearer <token>` 头。
2. OpenClaw 根据配置的 `audienceType` 和 `audience` 验证 Token：
...
- [表情回应](/tutorials/tools/reactions)
