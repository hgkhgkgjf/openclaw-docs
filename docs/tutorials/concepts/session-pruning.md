...
title: "会话修剪"
sidebarTitle: "会话修剪"
description: "说明 OpenClaw 的会话修剪机制：在 Anthropic 缓存 TTL 过期后临时裁剪旧工具结果，降低重新缓存成本。"
---
...
- OAuth 或 setup-token 配置文件：启用 `cache-ttl` 修剪，并把心跳设置为 `1h`。
- API Key 配置文件：启用 `cache-ttl` 修剪，把心跳设置为 `30m`，并在 Anthropic 模型上把 `cacheControlTtl` 默认为 `1h`。
...
- 哪部分变便宜：TTL 过期后的第一个请求，`cacheWrite` 通常会变小。
- 为什么 TTL 重置重要：修剪运行后，缓存窗口重新开始，后续请求可以复用新的缓存提示。
