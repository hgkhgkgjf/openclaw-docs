...
引导启动是首次运行时的初始化流程，用于准备智能体（Agent）工作区（Workspace）并收集身份信息。它在入门引导完成后、智能体（Agent）首次启动时执行。
...
引导启动始终在网关（Gateway）主机上运行。如果 macOS 应用连接到远程网关（Gateway），则工作区（Workspace）和引导文件位于该远程机器上。
...
- macOS 应用入门引导：[入门引导](/tutorials/getting-started/onboarding)
- 工作区（Workspace）布局：[智能体（Agent）工作区](/tutorials/concepts/agent-workspace)
