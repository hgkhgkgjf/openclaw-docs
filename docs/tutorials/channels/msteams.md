...
状态：支持文本、私聊附件和 Adaptive Cards 投票。频道/群组文件发送需要 `sharePointSiteId` + Microsoft Graph 权限（参见[在群聊中发送文件](#在群聊中发送文件)）。文件优先发送可使用消息操作里的 `upload-file`。
...
```text
先跑通控制 UI，然后接 Telegram 或 Discord，最后再接 Teams
```
...
普通用户不要固定插件版本，直接使用上面的包名即可跟随当前官方发布线。
...
建议先用 Microsoft Teams CLI 生成应用、Bot 注册和凭证。这样可以少走很多 Azure Portal 页面。
...
- `channels.msteams.allowFrom` 推荐使用稳定的 AAD 对象 ID，或静态访问组，例如 `accessGroup:core-team`。
...
- `channels.msteams.groupAllowFrom` 控制哪些发送者或静态访问组可以在群聊/频道中触发（回退到 `channels.msteams.allowFrom`）。
...
3. 点击 Review + create，再点击 Create（等待约 1-2 分钟）

### 步骤 2：获取凭证

1. 前往你的 Azure Bot 资源，打开配置页。
2. 复制 Microsoft App ID，这是你的 `appId`。
3. 点击管理密码，前往应用注册。
4. 在证书和密钥中新建客户端密钥，复制值，这是你的 `appPassword`。
5. 前往概述页，复制目录（租户）ID，这是你的 `tenantId`。

### 步骤 3：配置消息端点

1. 在 Azure Bot 中打开配置页。
2. 将消息端点设置为你的 Webhook URL：
   - 生产环境：`https://your-domain.com/api/messages`
   - 本地开发：使用隧道（参见下方[本地开发](#本地开发隧道)）

### 步骤 4：启用 Teams 通道

1. 在 Azure Bot 中打开通道页。
2. 点击 Microsoft Teams，配置后保存。
3. 接受服务条款
...
3. 前往应用功能，再打开机器人。
4. 选择手动输入机器人 ID 并粘贴你的 Azure Bot App ID
5. 勾选范围：个人、团队、群聊
6. 点击分发，下载应用包。
7. 在 Teams 中打开应用，进入管理你的应用，上传自定义应用并选择 ZIP。
...
### Azure Web Chat（先验证 Webhook）

1. 在 Azure Portal 中打开你的 Azure Bot 资源，进入“在 Web Chat 中测试”。
2. 发送一条消息。正常情况下应该能看到回复。
3. 这可以先确认 Webhook 端点可用，再处理 Teams 安装问题。
...
3. 重新打包清单和图标（`manifest.json`、`outline.png`、`color.png`）
4. 上传新的 ZIP：
...
2. 为租户授予管理员同意。
3. 递增 Teams 应用清单版本，重新上传，并在 Teams 中重新安装应用。
...
- `channels.msteams.allowFrom`：私聊允许列表（AAD 对象 ID、静态访问组、UPN 或显示名称）。当 Graph 访问可用时，向导在设置期间将名称解析为 ID。
...
- 会话（Session）键遵循标准智能体（Agent）格式（参见 [/concepts/session](/tutorials/concepts/session)）：
...
| 样式                  | 描述                                        | 推荐 `replyStyle`    |
| --------------------- | ------------------------------------------- | -------------------- |
| Posts（经典）      | 消息以卡片形式显示，回复在下方以线程形式展开     | `thread`（默认）      |
