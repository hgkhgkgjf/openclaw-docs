...
广播组允许多个智能体（Agent）处理同一条消息。当前主要用途是在一个 WhatsApp 群组或私信里，让不同职责的智能体分别给出结果，同时仍然使用同一个电话号码。
...
结果：当 OpenClaw 在此聊天中回复时，会运行这三个智能体。
...
注意：广播组不会绕过通道白名单或群组激活规则（提及/命令等）。它只改变符合处理条件的消息会交给哪些智能体。
...
建议让每个智能体只负责一件事，例如格式化、测试或安全检查。不要把所有任务都塞进一个通用的 `dev-helper`。
...
- [多智能体配置](/tutorials/tools/multi-agent-sandbox-tools)
- [路由配置](/tutorials/channels/channel-routing)
- [会话管理](/tutorials/concepts/sessions)
