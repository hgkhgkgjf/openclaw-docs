...
心跳就是让 AI 助手隔一段时间自己醒来一次，看看有没有该提醒你的事。
...
1. 先保持默认频率：通常每 `30m` 一次；某些 Anthropic 登录方式会是 `1h`。
2. 如果你想让它检查固定内容，可以在工作区写一个 `HEARTBEAT.md` 小清单。
3. 如果你想让提醒发到某个聊天软件，再设置 `target`。
...
- 后台任务："Consider outstanding tasks" 提示智能体（Agent）审查
  后续工作（收件箱、日历、提醒、排队工作）并报告紧急事项。
- 人工签到："Checkup sometimes on your human during day time" 提示
  偶尔发送轻量的"需要什么帮助吗？"消息，但通过使用你配置的
  本地时区（参阅 [/concepts/timezone](/tutorials/concepts/timezone)）避免夜间打扰。
...
```text
请检查 HEARTBEAT.md 里的任务。
如果没有需要提醒我的事情，只回复 HEARTBEAT_OK。
如果有紧急事项，用三句话以内告诉我。
```
...
- 在心跳运行期间，当 `HEARTBEAT_OK` 出现在回复的开头或结尾时，OpenClaw 将其视为确认。该 Token 会被去除，如果剩余内容 ≤ `ackMaxChars`（默认：300），回复将被丢弃。
- 如果 `HEARTBEAT_OK` 出现在回复的中间，它不会被特殊处理。
- 对于警报，不要包含 `HEARTBEAT_OK`；仅返回警报文本。

在心跳之外，消息开头/结尾的 `HEARTBEAT_OK` 会被去除
并记录；仅包含 `HEARTBEAT_OK` 的消息会被丢弃。

你可以把 `HEARTBEAT_OK` 理解成“今天没事，不用叫醒主人”。
OpenClaw 看到它后，默认不会把这类无事消息发给你，避免刷屏。
...
安全提示：不要在 `HEARTBEAT.md` 中放置密钥（API 密钥、电话号码、私有 Token），因为它会成为提示上下文的一部分。
