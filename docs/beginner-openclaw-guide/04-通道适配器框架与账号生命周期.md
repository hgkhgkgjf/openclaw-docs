...
2. 插件 manifest 声明：`extensions/<channel>/openclaw.plugin.json`
3. 插件注册通道能力：`src/plugins/registry.ts`
4. 网关启动/停止通道：`src/gateway/server-channels.ts`
5. 入站消息路由到 agent：`src/routing/resolve-route.ts`
6. 出站消息走统一适配器：`src/infra/outbound/deliver.ts`
7. 通道状态探测与体检：`src/gateway/server-methods/channels.ts`
...
### 模块 2：插件如何声明并注册“我是一个通道”

最新版要分两步看：

1. `openclaw.plugin.json` 先声明插件 ID、能力、配置 schema 和启用提示。
2. 插件运行时代码再通过 SDK 注册真正的 channel adapter。
