...
配置热重载系统监听文件变化，将 diff 路径映射为"热更/重启/忽略"计划，再执行对应动作，避免因小改动导致整个 gateway 重启。
...
const BASE_RELOAD_RULES: ReloadRule[] = [
  ...
  { prefix: "agent.heartbeat", kind: "hot", actions: ["restart-heartbeat"] }, // 旧配置兼容；新配置写 agents.defaults.heartbeat
