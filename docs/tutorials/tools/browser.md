...
title: "浏览器工具"
sidebarTitle: "浏览器工具"
description: "说明 OpenClaw 浏览器工具如何控制真实浏览器、配置本地或远程浏览器、截图、等待页面和连接 CDP 调试。"
---

# 浏览器工具（Browser Tool）

浏览器工具让 Agent 控制真实浏览器：打开网页、点击元素、填写表单、截图。它适合需要登录、跳转、等待页面渲染的网页任务。
...
通过 [Browserless](https://browserless.io) 等云端浏览器服务运行。适合服务器部署、无 GUI 环境，或需要并发浏览器任务的场景。
...
::: tip Browserless 集成
使用 Browserless 时，需要注册账号并获取 API Token。云端浏览器不占用本地桌面资源，适合长期自动化任务。
:::
