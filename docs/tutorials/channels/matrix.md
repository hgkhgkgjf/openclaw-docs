...
Matrix 是一个开放的去中心化消息协议。OpenClaw 以 Matrix 用户身份连接到任意 homeserver，因此你需要一个 Matrix 账户作为机器人。登录后，你可以直接私聊机器人或将其邀请到房间（Matrix 中的"群组"）。Beeper 也是一个有效的客户端选择，但需要启用 E2EE。
...
如果你只看到一个较旧的 token-hash 目录，这通常只是 Token 轮换后的正常连续性路径，不一定是故障。
但如果日志里出现类似 `matrix: multiple populated token-hash storage roots detected`，就说明同一账户下可能积累了多个仍然有内容的旧根目录。

这时不要急着删。更稳妥的做法是：

1. 先确认当前活跃根目录工作正常
2. 把明显过期的兄弟目录移到 `_archive/`
3. 观察一段时间无误后，再考虑删除

直接清空旧目录，容易把还没确认完的加密状态一起删掉。
...
- 环境变量（`MATRIX_HOMESERVER`、`MATRIX_ACCESS_TOKEN` 等）仅适用于默认账户。
