...
- `src/gateway/server-lanes.ts`：配置到并发数的映射
...
排队期间每次 drain 循环都会检查 `if (waitedMs >= entry.warnAfterMs)`，命中后调用 `onWait` 和 `diag.warn`。
...
`generation` 用来防止"热重启后旧 finally 块写脏状态"。
...
导致 `activeTaskIds` 残留旧条目。`resetAllLanes()` 通过递增 `generation` 和 `activeTaskIds.clear()` 强制恢复。
