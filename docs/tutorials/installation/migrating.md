...
本指南说明如何把 OpenClaw Gateway 从一台机器迁移到另一台机器，通常不需要重新引导。

迁移时要带走两类数据：

- 状态目录（`$OPENCLAW_STATE_DIR`，默认：`~/.openclaw/`）：包含配置、认证、会话和通道状态。
- 工作区（Workspace，默认 `~/.openclaw/workspace/`）：包含智能体文件、记忆和提示词。

常见问题通常出在 profile 不一致、文件权限错误，或只复制了配置文件而没有复制完整状态目录。
...
在新机器上，安装 CLI（如需要也安装 Node）：

- 参见：[安装](/tutorials/installation/)

在此阶段，引导创建一个新的 `~/.openclaw/` 是可以的，下一步会用旧机器的数据覆盖它。
...
如果旧网关使用了 profile（或 `OPENCLAW_STATE_DIR`），而新网关使用了不同的值，通常会看到这些症状：

- 配置更改不生效
- 通道缺失 / 已登出
- 空的会话历史

修复：用迁移过来的同一套 profile 或状态目录运行网关服务，然后重新运行：
...
应迁移整个 `$OPENCLAW_STATE_DIR` 文件夹。

### 陷阱：权限 / 所有权

如果你以 root 身份复制或更改了用户，网关可能无法读取凭据/会话。

修复：确保状态目录和工作区的所有者是运行网关的用户。

### 陷阱：在远程/本地模式之间迁移

- 如果你的 UI（WebUI/TUI）指向远程网关，远程主机拥有会话存储 + 工作区。
- 迁移你的笔记本电脑不会移动远程网关的状态。

如果你处于远程模式，请迁移网关主机，而不是只迁移本地笔记本。
...
- [OpenClaw 将数据存储在哪里？](/tutorials/help/faq)
