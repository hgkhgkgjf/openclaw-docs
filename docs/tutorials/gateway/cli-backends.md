...
OpenClaw 可以运行本地 AI CLI 作为 API 提供商（Provider）不可用、
被限速或暂时异常时的纯文本回退。这是有意保守的设计：

- 默认不直接注入 OpenClaw 工具。
- 支持 `bundleMcp: true` 的后端可以通过 loopback MCP bridge 接收 Gateway 工具。
- 文本输入 → 文本输出（可靠）。
...
如果你要的是完整 harness runtime、ACP 会话控制、后台任务和长期外部编码会话，请看
[ACP Agents](/tutorials/tools/acp-agents)。CLI 后端不是 ACP。

---

## 初学者快速入门

你可以无需任何配置使用 Claude Code CLI（OpenClaw 内置默认配置）：
...
如果你在默认模型或 `agents.defaults.cliBackends` 里显式引用某个捆绑 CLI 后端，
新版 OpenClaw 会自动加载拥有这个后端的捆绑插件。
...
每个条目以提供商（Provider）id 为键（例如 `claude-cli`、`my-cli`）。
