...
如果所有人都挤在同一个房间聊天，AI 可能把 Alice 的私人信息讲给 Bob。
所以 OpenClaw 需要决定：哪些消息放在同一个房间，哪些消息必须分开。

新手只要先记住：

- 只有你一个人用时，默认主会话通常够用。
- 多个人都能私信你的 AI 时，一定要把会话按人隔离。
- 群聊、频道、私聊最好各有自己的会话，不要混在一起。
...
- 你可以使用 `openclaw security audit` 验证你的 DM 设置（参见[安全](/tutorials/gateway/security)）。
...
翻成人话：以 Gateway 看到的会话为准。
不要让每个客户端自己猜“现在有哪些会话、用了多少上下文”。
...
OpenClaw 默认在 LLM 调用之前从内存上下文中修剪 旧的工具结果。这 不会 重写 JSONL 历史。参见 [/concepts/session-pruning](/tutorials/concepts/session-pruning)。

---

## 压缩前记忆刷新

当会话接近自动压缩时，OpenClaw 可以运行一次 静默记忆刷新 轮次，提醒模型将持久笔记写入磁盘。仅在工作区可写时运行。参见[记忆](/tutorials/concepts/memory)和[压缩](/tutorials/concepts/compaction)。

---

## 传输映射 : 会话键

下面这些键是 OpenClaw 内部用来区分“哪个聊天房间”的名字。
你不需要背格式；只有在查日志、迁移旧数据或做高级配置时才会用到。
