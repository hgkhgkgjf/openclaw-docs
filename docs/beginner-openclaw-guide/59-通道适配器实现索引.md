...
说明：最新版 OpenClaw 的通道大多位于 `extensions/<channel>/`，通过插件 manifest + SDK 能力注册接入 Gateway。下面的“核心文件”用的是实现职责说明；如果你在源码里查找，请优先从对应 `extensions/<channel>/` 目录开始。
...
| LINE | Webhook (HTTP) | 文本、图片、位置、Flex、模板、快速回复 | 最精细的消息类型：Flex Message 模板（10+种）、Rich Menu CRUD、Markdown到LINE 转换器、加载动画 keepalive |
...
- 最精细的消息类型支持
- Markdown 表格 到 Flex Bubble 自动转换
...
- `send.ts` : 发送消息 + Markdown 到 Signal 原生样式
...
## iMessage (`extensions/imessage/` 或 BlueBubbles 插件)
