...
description: "在 OpenClaw 中接入 MiniMax M2.1：OAuth、API Key、后备模型、本地 LM Studio、配置项和常见错误。"
---

# MiniMax

MiniMax 提供 M2/M2.1 模型系列。当前面向编程场景的版本是 MiniMax M2.1（2025 年 12 月 23 日发布）。
...
MiniMax 在发布说明里重点提到这些改进：

- 多语言编程能力更强，覆盖 Rust、Java、Go、C++、Kotlin、Objective-C、TS/JS。
- Web 和应用开发输出更好，也更关注界面质量，包括原生移动端。
- 复合指令处理能力增强，更适合办公类工作流。
- 回复更短，Token 用量更低，迭代更快。
- 对工具和智能体框架的兼容性更好，包括 Claude Code、Droid/Factory AI、Cline、Kilo Code、Roo Code、BlackBox。
- 对话和技术写作质量提升。
...
- 速度：Lightning 是 MiniMax 定价文档里的快速变体。
- 成本：定价显示两者输入成本相同，Lightning 的输出成本更高。
- 编程计划路由：MiniMax 编程计划不能直接指定 Lightning 后端。MiniMax 会把多数请求自动路由到 Lightning，流量高峰时再回退到常规 M2.1 后端。
...
推荐先试这个方式。它通过 OAuth 配置 MiniMax 编程计划，不需要手动复制 API Key。
...
也可以把 Opus 4.6 作为主模型，MiniMax M2.1 作为故障转移模型。
...
如果你要用 LM Studio 做本地推理，可以手动加一个本地 provider。这个方案更适合性能较强的台式机或服务器。
...
- 提供商规则请参见 [/concepts/model-providers](/tutorials/concepts/model-providers)。
...
这通常表示 MiniMax provider 没配上：既没有 provider 条目，也没有找到 MiniMax 认证配置文件或环境变量。相关检测修复包含在 2026.1.12 中（本文撰写时尚未发布）。处理方式：
