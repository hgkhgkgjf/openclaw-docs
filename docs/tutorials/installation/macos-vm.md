...
- 小型 Linux VPS 用于始终在线的网关（Gateway），且成本低。参见 [远程访问](/tutorials/gateway/remote)。
- 专用硬件（Mac mini 或 Linux 主机），如果你需要完全控制和住宅 IP 用于浏览器自动化。许多网站会阻止数据中心 IP，因此本地浏览通常效果更好。
- 混合方案： 将网关部署在便宜的 VPS 上，需要浏览器/UI 自动化时将你的 Mac 作为节点连接。参见 [节点](/tutorials/nodes/) 和 [网关远程](/tutorials/gateway/remote)。
...
2. 使用你的 Apple ID 登录
3. 启用 Web API 并设置密码
4. 将 BlueBubbles Webhook 指向你的网关（示例：`https://your-gateway-host:18789/bluebubbles-webhook?password=<password>`）
...
完整设置详情：[BlueBubbles 通道](/tutorials/channels/bluebubbles)
...
要实现真正的始终在线，考虑专用 Mac mini 或小型 VPS。参见 [远程访问](/tutorials/gateway/remote)。
...
- [节点](/tutorials/nodes/)
- [网关远程](/tutorials/gateway/remote)
- [BlueBubbles 通道](/tutorials/channels/bluebubbles)
...
- [Docker 沙箱](/tutorials/installation/docker)（替代隔离方案）
