...
本页说明如何在 [Fly.io](https://fly.io) 机器上运行 OpenClaw Gateway，并配置持久化存储、自动 HTTPS 和常见通道访问。
...
安全说明：默认配置会暴露公共 URL。要获得无公共 IP 的部署，请参见 [私有部署](#私有部署加固) 或使用 `fly.private.toml`。
...
- 像对待密码一样对待这些 Token。
- 优先使用环境变量存放 API 密钥和 Token，不要把密钥写进 `openclaw.json`。
...
容器继续重启或被杀死。迹象：`SIGABRT`、`v8::internal::Runtime_AllocateInYoungGeneration`，或无声重启。
...
注意：512MB 太小。1GB 可能可以工作，但在负载或详细日志记录时仍可能 OOM。推荐 2GB。
...
默认情况下，Fly 会分配公共 IP，使你的网关可以通过 `https://your-app.fly.dev` 访问。这很方便，但也意味着你的部署可被互联网扫描器（Shodan、Censys 等）发现。
