...
把它说简单一点：你准备一个 Twitch 机器人账号，让它进入某个直播间聊天室。观众在聊天室里按规则喊它，它就把消息交给 OpenClaw，再把 AI 的回复发回聊天室。

::: warning 新手先确认这三件事
1. 控制 UI 已经能正常聊天。
2. Gateway 正在运行。
3. 你已经准备好 Twitch Bot Token，并且 Token 有 `chat:read` 和 `chat:write` 权限。

如果第 1 件还没完成，请先回到[零基础照着做](/tutorials/getting-started/grandma-guide)。
:::
...
Twitch 通过插件提供。当前版本可能已随 OpenClaw 一起提供，也可能由向导提示你安装；如果你的环境里没有它，再按下面方式手动安装。
...
如果你不知道结果是什么意思，先只看两件事：

- Twitch 是否显示为 enabled 或 connected。
- 报错里是否出现 token、scope、channel、permission 这些词。
