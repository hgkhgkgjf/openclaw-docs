...
When signing with `SIGN_IDENTITY="-"` (ad-hoc), the script automatically disables the Hardened Runtime (`--options runtime`). This is necessary to prevent crashes when the app attempts to load embedded frameworks (like Sparkle) that do not share the same Team ID. Ad-hoc signatures also break TCC permission persistence; see [macOS permissions](/tutorials/platforms/mac/permissions) for recovery steps.
...
- [macOS permissions](/tutorials/platforms/mac/permissions)
