...
设计目标是将所有网络发现/广播保留在节点网关（Gateway）（`openclaw gateway`）中，让客户端（mac 应用、iOS）作为消费者。
...
- 旧版 TCP 桥接（已弃用/移除）：旧的节点传输（参阅[桥接协议](/tutorials/gateway/bridge-protocol)）；不再用于发现广播。

协议详情：

- [网关（Gateway）协议](/tutorials/gateway/protocol)
- [桥接协议（旧版）](/tutorials/gateway/bridge-protocol)
...
故障排查和信标详情：[Bonjour](/tutorials/gateway/bonjour)。
...
- Bonjour/mDNS TXT 记录是未经认证的。客户端必须仅将 TXT 值视为用户体验提示。
- 路由（主机/端口）应优先使用解析的服务端点（SRV + A/AAAA）而非 TXT 提供的 `lanHost`、`tailnetDns` 或 `gatewayPort`。
- TLS 固定不得允许广播的 `gatewayTlsSha256` 覆盖之前存储的固定值。
- iOS/Android 节点应将基于发现的直连视为仅限 TLS，并在存储首次固定值之前要求明确的"信任此指纹"确认（带外验证）。
...
参阅[远程访问](/tutorials/gateway/remote)。
...
- 配对请求在网关（Gateway）中创建/批准/拒绝（参阅[网关（Gateway）配对](/tutorials/gateway/pairing)）。
- 网关（Gateway）强制执行：
  - 认证（Token / 密钥对）
