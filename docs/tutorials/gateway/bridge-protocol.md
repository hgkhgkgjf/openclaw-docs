...
桥接协议是一种旧版节点传输协议（TCP JSONL）。新的节点客户端
...
节点可以发出 `exec.finished` 或 `exec.denied` 事件来记录 system.run 活动。
...
- Bonjour 不会跨网络；需要时使用手动 host/port 或广域 DNS-SD。

---

## 版本控制

桥接当前为隐式 v1（无 min/max 协商）。预期向后兼容；
